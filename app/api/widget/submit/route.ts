...
export async function POST(request: Request) {
 ...
 .onConflictDoNothing({ target: [widgetLeads.email, widgetLeads.courseId] })
 .returning();

 if (!lead) {
 const [existing] = await db
 .select({ id: widgetLeads.id, convertedToStudent: widgetLeads.convertedToStudent, razorpayOrderId: widgetLeads.razorpayOrderId })
 .from(widgetLeads)
 .where(and(eq(widgetLeads.email, data.email), eq(widgetLeads.courseId, courseId)))
 .limit(1);

 if (!existing) {
 return widgetJson(ctx, request, { error: "SUBMIT_FAILED", message: "Could not create lead. Please try again." }, 500);
 }
 ...
 return widgetJson(ctx, request, { leadId: existing.id, alreadyInitiated: true, razorpayOrderId: existing.razorpayOrderId }, 200);
 }
